-module(cache_tls_acceptor2).
-export([init/1, terminate/2]).
-export([handle_call/3, handle_cast/2, handle_info/2, handle_continue/2]).
-behavior(gen_server).
-record(state, { listener_sock = undefined, accept_sock = undefined, tls_sock = undefined }).
...
handle_continue(accepted, #state{ accept_sock = AcceptSock } = State) ->
    case ssl:handshake(AcceptSock, ?TIMEOUT) of
        {ok, TlsSock} ->
            {noreply, State#state{ tls_sock = TlsSock }, {continue, wait_command}};
        Otherwise -> 
